...
function ReconnectingWebSocket(a) {
	function f(g) {
		...
		if (b.debug || ReconnectingWebSocket.debugAll) {
			window.console.log("ReconnectingWebSocket", "attempt-connect", a)
		}
		var h = c;
		var i = setTimeout(function() {
			if (b.debug || ReconnectingWebSocket.debugAll) {
				window.console.log("ReconnectingWebSocket", "connection-timeout", a)
			}
			e = true;
			h.close();
			e = false
		...
		c.onopen = function(c) {
			clearTimeout(i);
			if (b.debug || ReconnectingWebSocket.debugAll) {
				window.console.log("ReconnectingWebSocket", "onopen", a)
			}
		...
		c.onclose = function(h) {
			...
			if (d) {
				...
				if (!g && !e) {
					if (b.debug || ReconnectingWebSocket.debugAll) {
						window.console.log("ReconnectingWebSocket", "onclose", a)
					}
					b.onclose(h)
				}
				setTimeout(function() {
		...
		c.onmessage = function(c) {
			if (b.debug || ReconnectingWebSocket.debugAll) {
				window.console.log("ReconnectingWebSocket", "onmessage", a, c.data)
			}
			b.onmessage(c)
		};
		c.onerror = function(c) {
			if (b.debug || ReconnectingWebSocket.debugAll) {
				window.console.log("ReconnectingWebSocket", "onerror", a, c)
			}
	...
	this.send = function(d) {
		if (c) {
			if (b.debug || ReconnectingWebSocket.debugAll) {
				window.console.log("ReconnectingWebSocket", "send", a, d)
			}
			return c.send(d)
		} else {
